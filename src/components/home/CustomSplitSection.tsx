    ...
    >
      <div className="mx-auto grid max-w-6xl items-center gap-8 px-6 py-10 md:grid-cols-2 md:gap-12 md:py-16">
        ...
        <div className={`flex flex-col gap-4 ${imageRight ? 'md:order-1' : 'md:order-2'}`}>
          ...
          {body && (
            <div
              className="text-[calc(14px_*_var(--font-scale,1))] leading-relaxed opacity-80 md:text-[calc(15px_*_var(--font-scale,1))] [&_a]:underline [&_p]:mb-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5"
          ...
          {c.button_text && c.button_link && (
            <SectionLink
              to={c.button_link}
              className="mt-2 inline-flex w-fit items-center border border-current px-8 py-3.5 text-[calc(14px_*_var(--font-scale,1))] font-medium transition-opacity hover:opacity-75"
            >
              {c.button_text}
            </SectionLink>
          )}
